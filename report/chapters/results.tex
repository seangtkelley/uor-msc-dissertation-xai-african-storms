...
Finally, as coarse representation of the general climatology of the region, Figure \ref{fig:figures} shows the mean values of meteorological variables across the study region which are relevant in the explainability analyses below.
\begin{figure}
\centering
...
\acrfull{tcwv}'s feature contribution for the model trained on only \acrshort{era5} data is quite peculiar. On the one hand, the fact it has strong contributions to both models should be expected based on established literature \citep{Li2023,Muetzelfeldt2025,Klein2020}. However, the relationship between the its contribution and feature value should theoretically be reversed. According to the the beeswarm plot d) in Figure \ref{fig:obs_intensification_summary}, higher values of mean \acrshort{tcwv} tend to slow intensification where as low values accelerate it.

Looking at the spatial and temporal distribution of mean \acrshort{tcwv} values and their corresponding SHAP values in Figure \ref{fig:obs_intensification_era5_shap_mean_tcwv_map_by_hour} sheds light on what might be driving this unexpected relationship. As can be seen, low values of mean \acrshort{tcwv} are generally concentrated over the Ethiopian Highlands and surrounding elevated terrain. This relationship is confirmed in the correlation heatmap in Appendix \insertref{appendix corr heatmap} where mean \acrshort{tcwv} has a correlation coefficient of -0.59 with elevation.

...
other key observations include: \todo{make a paragraph}
\begin{itemize}
    \item the longer the storm is over the ocean, the more it tends to intensify from c)
    \item skin temperature is a critical factor in storm intensification, as indicated by its consistent presence among the top features for both models from c) and d). higher skin temperatures are associated with increased intensification, likely due to enhanced surface heating and moisture availability.
    \item no clear diurnal cycle
    \item \acrshort{cape} is much less important than with predicting storm max intensity from d)
    \item counter-intuitive importance between \acrshort{cape} and domain mean \acrshort{cape}
\end{itemize}
...
other key observations include: \todo{make a paragraph}
\begin{itemize}
    \item \acrfull{bt} is not a top feature
    \item strong diurnal cycle with peak precipitation during midday
    \item high importance of mid and upper level humidity
    \item again counter-intuitive relationship between \acrshort{cape} and domain mean \acrshort{cape}
    \item \acrshort{olr} is important, likely as a proxy for cloud cover
\end{itemize}
